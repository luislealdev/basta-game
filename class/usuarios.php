...
class Usuario extends BaseDeDatos{
    function action($cual){
        ...
        switch($cual){
            ...
            case 'formNew':
                ...
                <form method='post' class='col-md-4 border border-dark border-3 p-3 mx-auto bg-info'>".
    ...
    /*function despTablaDatos($query){
        ...
        $header = '<span class = "badge bg-info">' . $tablaN . '<i class="bi bi-plus"></i> </span>';
        //fin cabecera

        //iconos de accion
        $datos .= '<td class="col-1"><button class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></button></td>';
        $datos .= '<td class="col-1"><button class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i></button></td>';

        foreach ($this->a_bloqRegistros as $row){
            $datos .='<tr>';
            foreach ($row as $perro)
                $datos .= "<td>" . $perro . "</td> ";
            $datos .= "</tr>";
        }
        $datos .= '</table></div>';
